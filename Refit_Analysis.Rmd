...
features_2017 <- read_csv("features-hamby.csv")

features_2017 <- features_2017 %>%
  select(-compare_id, -profile1_id, -profile2_id, -land1_id, -land2_id) %>%
    filter(study1 != "Cary" & study2 != "Cary") %>%
      rename(BarrelB = barrel1, BulletB = bullet1, LandB = land1) %>%
      rename(BarrelA = barrel2, BulletA = bullet2, LandA = land2) %>%
        select(-study1) %>%
      rename(Set = study2) %>%
          mutate(Set = gsub("Hamby44", "Hamby173", Set)) %>%
            mutate(Set = factor(Set, levels = c("Hamby173", "Hamby252"))) %>%
      rename(ccf2 = ccf, rough_cor2 = rough_cor, lag2 = lag, D2 = D, sd_D2 = sd_D, signature_length2 = signature_length, overlap2 = overlap, matches2 = matches, mismatches2 = mismatches, cms2x = cms, non_cms2 = non_cms, sum_peaks2 = sum_peaks)
...
features_2017 <- within(features_2017, BulletA[BulletA == 1 & BarrelA == "A"] <- "A")
...
features_2017$BarrelA[features_2017$BarrelA == "A"] <- "Unknown"
...
features_2017 <- features_2017 %>% 
  mutate(Bullet1 = paste(Set, BarrelB, BulletB, LandB, sep = "-"),
         Bullet2 = paste(Set, BarrelA, BulletA, LandA, sep = "-"))


features_2017 <- features_2017[order(features_2017$Set), ]
...
```{r}
Feature_Comparisons <- inner_join(comparisons, features_2017)
...
Feature_Comparisons %>% 
  ggplot(aes(x = rough_cor, y = rough_cor2))+
    geom_point()

Feature_Comparisons %>% 
  ggplot(aes(x = lag, y = lag2))+
    geom_point()
...
Feature_Comparisons %>% 
  ggplot(aes(x = sd_D, y = sd_D2))+
    geom_point()

Feature_Comparisons %>% 
  ggplot(aes(x = signature_length, y = signature_length2))+
    geom_point()

Feature_Comparisons %>% 
  ggplot(aes(x = overlap, y = overlap2))+
    geom_point()

Feature_Comparisons %>% 
  ggplot(aes(x = matches, y = matches2))+
    geom_point()

Feature_Comparisons %>% 
  ggplot(aes(x = mismatches, y = mismatches2))+
    geom_point()
...
Feature_Comparisons %>% 
  ggplot(aes(x = non_cms, y = non_cms2))+
    geom_point()

Feature_Comparisons %>% 
  ggplot(aes(x = sum_peaks, y = sum_peaks2))+
    geom_point()
