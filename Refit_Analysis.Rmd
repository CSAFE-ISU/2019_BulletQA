...
## There is some noticeable missing data for Signature comparisons [Hamby252/Bullet1]. Looking for noticeable differences, we see that there is missing values when plotting the Bullet 1 and Bullet 2.  This is hard to see when plotting the Lands.
```{r}
...
## Looking at the land-land matching scores for bullets 1 and 2. We plotted an example when the bullets came from the same barrel and when the bullets came from different barrels. The matrixs shown for bullets 1 and 2 that come from the same barrel looks promising but rfscores seem lower than expected.
```{r}
library(gridExtra)
head(comparisons)
  ...
  filter(BarrelA == 10 & BarrelB == 10) %>% 
    filter(BulletA == 1 & BulletB == 2 ) %>% 
      ggplot(aes(x = LandA, y = LandB, fill = rfscore))+ 
        geom_tile()+ 
          scale_fill_gradient2(low = "#000000", high = "#56B4E9", midpoint = 0.5) +facet_grid(BulletB~BulletA, labeller = "label_both")+ 
            xlab("Land A") + ylab("Land B") + theme(aspect.ratio = 1)+ ggtitle("Same Source but Different Bullet")+ facet_grid(~Set)
  ...
  filter(BarrelA == 10 & BarrelB == 5) %>% 
    filter(BulletA == 1 & BulletB == 2 ) %>% 
      ggplot(aes(x = LandA, y = LandB, fill = rfscore))+ 
        geom_tile()+ 
          scale_fill_gradient2(low = "#000000", high = "#56B4E9", midpoint = 0.5) +facet_grid(BulletB~BulletA, labeller = "label_both")+ 
            xlab("Land A") + ylab("Land B") + theme(aspect.ratio = 1)+ ggtitle("Different Source")+ facet_grid(~Set)
