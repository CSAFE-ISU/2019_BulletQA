...
length(features_2017$BarrelA)
length(features_2017$BarrelB)
```
 ...
 filter(!is.na(BarrelA)) %>%
   filter(!is.na(BarrelB)) %>%
     group_by(BarrelA, BarrelB, Bullet1, Bullet2, Set) %>%
      arrange(desc(signature_length2)) %>% summarize(count = n()) %>%
       filter(row_number() == 1) %>% 
                ungroup() %>% 
  group_by(BarrelA, BarrelB, Set) %>%
    summarise(count = n()) %>% 
...
Looking at features_2017 plot1 we can see that my analysis about duplicate data seems to be correct. 
Looking at features_2017 plot2 we can see that there is also missing data on the Barrel-Bullet-Land level.
Both of these visual representations are also written in the word.document 
...
#Everything past this point is for comparison data separatley.  There is still issues to be solved before Joining both data frames together.#
