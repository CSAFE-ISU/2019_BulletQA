...
Hamby_Data_Long_by_YEAR <- Joined_df %>% 
  select(1:2, match, ccf:sum_peaks, ccf2:sum_peaks2) %>% 
    gather(key="measure", value="value", ccf:sum_peaks2) %>%   
      mutate(Year = if_else(grepl("^.+(2|2x)$", measure), 2017, 2019)) %>% 
        mutate(measure = str_remove(measure, "2x?$"))
